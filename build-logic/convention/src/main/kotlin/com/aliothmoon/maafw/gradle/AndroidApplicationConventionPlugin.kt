...
/**
 * Release ABI set: -Pmaa.abi wins, then local.properties maa.abi, default both.
 * One of all|arm64-v8a|x86_64 (x64 accepted as an alias, comma list tolerated).
 * Per-ABI release packages pair this with setup_maa_framework.py --abi so the
 * packaged jniLibs and abiFilters always agree
 */
private fun Project.releaseAbiSetting(): List<String> {
    val raw = providers.gradleProperty("maa.abi").orNull
        ?: textSetting("maa.abi", "MAA_ABI")
        ?: ""
    val abis = raw.trim().split(',', ' ').map(String::trim).filter(String::isNotEmpty)
    if (abis.isEmpty() || abis.singleOrNull()?.equals("all", ignoreCase = true) == true) {
        return SHIPPED_ABIS
    }
    return abis.map { if (it.equals("x64", ignoreCase = true)) "x86_64" else it }
}
    ...
    override fun apply(target: Project) {
        with(target) {
            ...
            android.defaultConfig {
                ...
                versionCode = textSetting("build.versionCode", "BUILD_VERSION_CODE")?.toIntOrNull()
                    ?: gitVersionCode()
