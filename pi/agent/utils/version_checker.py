...
MIRRORCHYAN_API_BASE = "https://mirrorchyan.com/api"
...
def check_resource_version(interface_file_path: str = "./interface.json") -> dict[str, Any]:
    ...
    try:
        ...
        # 调用API检查最新版本：主机固定为 mirrorchyan.com，rid/查询参数全部编码
        api_url = f"{MIRRORCHYAN_API_BASE}/resources/{quote(str(rid), safe='')}/latest/"
        if not api_url.startswith(f"{MIRRORCHYAN_API_BASE}/"):
            raise ValueError(f"非法的版本检查地址: {api_url}")
